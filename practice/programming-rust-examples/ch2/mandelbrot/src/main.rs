        ...
        crossbeam::scope(|spawner| {
            for (i, band) in bands.into_iter().enumerate() {
                ...
                let band_lower_right = 
                    pixel_to_point(bounds, (bounds.0 - 1, top + height), upper_left, lower_right);
...
fn test_parse_complex() {
    assert_eq!(parse_complex("1.0,-1.0"), Some(Complex {re: 1.0, im: -1.0}));
...
fn test_parse_pair() {
    ...
    assert_eq!(parse_pair::<f64>(".5x", ','), None);
